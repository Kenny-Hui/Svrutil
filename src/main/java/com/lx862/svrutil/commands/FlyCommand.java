...
public class FlyCommand {
    ...
    private static int execute(CommandContext<ServerCommandSource> context, ServerPlayerEntity player) {
        if (player.getAbilities().allowFlying) {
            ...
            player.sendMessage(Text.literal("You can no longer fly").formatted(Formatting.RED), false);
        } else {
            player.getAbilities().allowFlying = true;
            player.sendMessage(Text.literal("You can now fly").formatted(Formatting.GREEN), false);
        }
